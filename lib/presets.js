...
function makePresetSaveForm() {
  ...
  let $presetId = makeInput("Entry identifier&nbsp;", {
  ...
  $presetId.append($presetIdTooltip);
...
function savePreset(info) {
  ...
  exports.presets[info.group][info.presetId] = presetData;
}

function writePresets() {
  let presetsCSON = CSON.stringify(exports.presets);
  fs.writeFile(path.join(__dirname, "..", "presets.cson"), presetsCSON, (err) => {
    if (err) {
      console.log(err);
    } else {
      console.log("Saved presets.");
      $("#saved-preset-message").text("Saved quick entry!");
    }
  });
}
