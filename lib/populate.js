...
exports.fillSidebar = function() {
  ...
  let homeBtn = `<a class="nav-group-item" id="home" onclick=populate.populate("home")>Home</a>`;
  let endBtns = `<a class="nav-group-item" id="data" onclick=populate.populate("data")>Data</a>
    <a class="nav-group-item" id="preferences" onclick=populate.populate("preferences")>Edit preferences</a>`;
  let pageBtns = pages.map((page) => {
    let pageBtn = `<a class="nav-group-item" id="${page}" \
      onclick=populate.populate("${page}")>${forms[page].pageTitle}</a>`;
    return pageBtn;
  }).join("\n");
