...
exports.populate = function(page) {
  // forms is already loaded
  // let formPath = path.join(__dirname, "..", "forms.cson");
    ...
    let promise = new Promise((resolve, reject) => {
      switch (page) {
        case "main":
          ...
          break;
        case "influences":
          resolve(buttonsInfluence);
          break;
        case "status":
          resolve(buttonsStatus);
          break;
        case "data":
          // dataProc.combineData(() => {
          //   resolve([dataProc.dataHTML]);
          // });
          dataProc.getFields()
            .then(resolve([dataProc.dataHTML()]));
          break;
        case "preferences":
          resolve([prefs.prefsHTML]);
          break;
        default:
          console.log(`Displaying home page.`);
          resolve([`<p>Please select a page from the sidebar.</p>`]);
      }
    });
    promise.then((btns) => {
      if (btns.length != 1) {
