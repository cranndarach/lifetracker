...
exports.loadData = function() {
  let dbPath = path.join(config.config.saveDir, "all-data.json");
  return jsonfile.readFileAsync(dbPath)
    .then((data) => {
...
exports.getTags = function() {
  ...
  return Promise.all(Promise.each(exports.data, (entry) => {
    let tags = entry.tags;
    if (!tags) { return; }
    tags = _.map(_.split(entry.tags, ","), _.trim);
    console.log(typeof tags);
    // } else if (tags instanceof String) {
    // tags = _.map(_.split(tags, ","), _.trim);
    // }
    console.log(tags);
    exports.updateTags(tags);
  }))
    .catch((err) => {
      console.log(err.stack);
    });
...
exports.updateTags = function(tags) {
  return Promise.all(Promise.each(tags, (tag) => {
    if (!_.includes(exports.tags, tag)) {
      exports.tags.push(tag);
    }
  }))
    .then(() => {
      exports.tagsObj = _.zipObject(exports.tags);
    });
};
