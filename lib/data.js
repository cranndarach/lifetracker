...
function matchCriteria(obj, criteria) {
  _.forEach(criteria, (searchText, field) => {
    if (obj[field].includes(searchText)) {
      return True;
    }
  });
}
...
exports.search = function() {
  // TODO: Figure out how to pair selected values with the search text using
  // Materialize's select elements. See `exports.selectInstances` above.
  let $queryRows = $("#filter .query-row");
  console.log($queryRows);
  let criteria = {};
  $queryRows.each(($row) => {
    console.log($row);
    let fields = $row.find("select").getSelectedValues();
    let searchText = $row.find("input").val();
  ...
  let results = _.filter(exports.data, (entry) => {
    return matchCriteria(entry, criteria);
  });
  exports.found = results;
  $("#data-space").html(tableify({data: results}))
